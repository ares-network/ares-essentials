package com.llewkcor.ares.essentials.support.menu;
...
public final class TicketMenu extends Menu {
    ...
    public TicketMenu(Essentials plugin, Player player, Collection<ISupport> tickets) {
        super(plugin, player, "Open Tickets", 6);
        ...
        for (int i = start; i < end; i++) {
            ...
            if (ticket instanceof Request) {
                ...
                addItem(new ClickableItem(icon, cursor, click -> {
                    ...
                    player.sendMessage(Essentials.PRIMARY + "Summoned to " + Essentials.SECONDARY + dest.getName());
                }));
            }
