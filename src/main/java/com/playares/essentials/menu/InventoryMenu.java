package com.playares.essentials.menu;
    ...
    private void update() {
        ...
        for (int i = 0; i < observed.getInventory().getSize(); i++) {
            ...
            addItem(new ClickableItem(item, i, click -> {
                ...
                observed.sendMessage(EssentialsService.SECONDARY + player.getName() + EssentialsService.PRIMARY + " transferred an item from your inventory to theirs");
                player.sendMessage(EssentialsService.SECONDARY + observed.getName() + EssentialsService.PRIMARY + "'s item has been transferred to your inventory");
            }));
        }
