package com.playares.essentials.staff.listener;
...
public final class StaffListener implements Listener {
    ...
    public void onLogin(AsyncPlayerPreLoginEvent event) {
        ...
        final StaffAccount account = manager.getAccountFromDatabase(Filters.eq("id", uniqueId));
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (player.hasPermission("essentials.staff")) {
            ...
            if (account.isEnabled(StaffAccount.StaffSetting.JOIN_VANISHED)) {
                manager.getEssentials().getVanishManager().getHandler().hidePlayer(player);
            }
        } else {
            ...
            if (account != null) {
                ...
                manager.deleteAccountFromDatabase(account);
            }
        }
    }
